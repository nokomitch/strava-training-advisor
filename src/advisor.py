...
**80/20ルール**: Z1+Z2で全トレーニング時間の80%以上を占めることが目標。
...
### Base期（次のAレース12週以上前）
- **Z1+Z2: 95%以上**（Z3は原則禁止、ADSなし・AeT/AnT差≤10%でも控える）
...
"""
    ...
    if dist.total_s > 0:
        zone_section += f"""- Zone 1 (回復・脂肪代謝): {dist.zone_pct(1):.1f}% ({dist.zone1_s/3600:.1f}h)
...
- 平均週間走行距離: {result.avg_weekly_km:.1f} km
- 低強度(Z1+Z2)割合: {result.overall_zone_distribution.low_intensity_pct:.1f}%（目標80%以上）
